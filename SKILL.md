...
反問時**必須使用下方模板、五種類別全部列出**（即使你已判斷最可能是其中兩種，仍列出全部五種，可在最可能的選項加註推薦與說明）：
...
轉換他平台策略時，**產出的 XS 程式碼區塊（含註解）不得殘留原平台語法字樣**（`strategy.entry`、`Buy`、`==` 等）— 下游使用者會整塊複製，殘留字樣會被一起帶進編輯器。新舊語法對照說明請放在程式碼區塊外的文字或表格。
