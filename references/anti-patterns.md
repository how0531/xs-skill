...
**✅ 官方正解（更好、更快，v7.06.01 起）**：`SetBarBack` 的**第二參數頻率形式**直接設「該頻率」的引用筆數，不必用主頻硬換算：

```xs
// 跑 5 分策略要 average(GetField("收盤價","D"), 20)：
setbarback(20, "D");    // 直接把「日頻」引用筆數設足 20 根
settotalbar(3);         // 主頻 5 分只要留一點，不必算 20×54=1080
```

官方實測 `SetBarBack(N, "頻率")` 的資料準備速度**比灌大 `SetTotalBar` 更快**，商品/跨日越多差距越大。優先用這個頻率形式，上面的「灌大主頻」是舊版沒有此語法時的退路。
