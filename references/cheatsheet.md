...
`GetField("內盤量", "5")` 讀盤中即時欄位時，較大頻率的值分兩種算法：

- **累積型**（內盤量/外盤量/上漲量/下跌量/成交金額/各級買賣單量）→ 回傳這 5 分鐘內各 1 分鐘的**加總**
- **期末型**（均價/估計量/理論價/波動率/隱含波動率）→ 回傳**最後 1 分鐘**的值

不指定頻率則依腳本執行頻率。這類欄位最小頻率 1 分鐘、只支援指標與策略雷達（不支援選股）。像「外資買賣超」一天才公布一次的欄位受資料源限制沒有分鐘頻率。

### 幾個欄位的非顯而易見語意（官方 lesson 補充）

- **控盤者/散戶門檻**：`控盤者買進張數` = 每檔、每分公司、每價位買進金額 **>100 萬**的張數；`散戶買張`/`散戶賣張` = 每檔、每分公司、每價位交易金額 **<50 萬**者。可據此自算散戶買進比例（記得 `if volume<>0`）。
- **收集派發指標 / 分公司家數差方向**：`收集派發指標`、「分公司單買單賣家數」的買賣家數差，官方一律以「**賣出家數 − 買進家數**」計算（不是買−賣）。**正值 = 大戶買進**（大戶集中少數分點大量買、賣壓分散多數分點），寫方向判斷別把符號設反。
- **融資維持率**官方公式 = `收盤價 / (融資成本線 × 0.6) × 100%`（`融資成本線` 是可 `GetField` 的真實欄位，0.6 即約 6 成融資成數）。融資/融券的「增減」在大盤顯示金額、個股顯示張數。
- **美股盤前盤後（.18.01）**：只在**分鐘頻率**有資料（日頻不含，故頻率設「日」時無法勾選），且此時段委託**只能限價單**。
...
| SDT 共享資料表 | 38 | `SDT_SetValue`/`SDT_GetValue`, `SDT_SetValueIf`(CAS 鎖), `SDT_Sort`, `SDT_Max`/`Min`/`Median`, `SDT_HasKey`（二維表；`_L`＝策略共享、無`_L`＝商品獨立；`_L` 僅**策略雷達/自動交易實盤**可用，指標/選股/所有回測不支援；上限 5000 列×100 行；完整平台表與 CSV 匯入見 xshelp/DIGEST.md SDT 段） |
