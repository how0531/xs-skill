...
| `合約乘數` | 契約值乘數（大台 200 / 小台 50） | Numeric | 台期貨、台選擇權 |
...
```xs
// 名目曝險估算（大台乘數 200），可用於保證金與風險控管
_Notional = Position * GetSymbolInfo("合約乘數") * Close;

// 轉倉：直接取次月合約代碼，不必自行組裝年月字串
_NextMonth = GetSymbolInfo("期貨遠月");     // 例：FITXN02.TF
```

⚠️ **`到期日` 支援期貨**：官方欄位總表把它與選擇權欄位並列，容易誤判成「僅限選擇權」而改寫成其他函數。另注意台指期的「最後交易日（第三個星期三）」與「最終結算日（次一營業日）」不是同一天，實作結算／轉倉前先 `Print` 出實際回傳值核對觸發日。
