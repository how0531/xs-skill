...
> ✅ 已確認為官方功能（XQ lesson「繪圖偏移說明」）：`shift:=N` 讓 `Plot`/`PlotN`/`PlotK`/`PlotFill` 以當根 K 棒為基準偏移（正=向右/未來、負=向左/過去，單位 K 棒數）。`PlotLine` 因本身可指定位置，**不支援** shift。每個 Plot 只能一個偏移值（給複數值會報錯）。
>
> **向右偏移畫不出來的陷阱**：正偏移超出「保留空間」不會畫；保留空間會隨 K 棒增加而縮減。到「技術分析設定 → 一般」勾「**固定保留空間**」使寬度固定不縮（最大 60），並確保設定的 K 棒數 > 偏移量。
...
## 11. 繪圖函數行為與座標（XQ lesson 教學站彙整）

> 錯誤代碼另見 `references/error-codes.md`（指標 1002–1043）。特別：`1040` = 指標腳本不支援 Quote 報價欄位語法（指標不能用 GetQuote/q_）；`1010` = 最大引用小於腳本需要範圍。

### PlotLine（畫直線／射線）

`PlotLine(序列編號, x1, y1, x2, y2 [, "名稱"])`：X = 第幾根 bar（1~CurrentBar）、Y = 價位；序列編號 1~999 與 Plot 共用（不可重複）。

- **預設每次呼叫會清掉同序列先前的 PlotLine** 並以本次起訖點取代 → 逐 bar 重畫只留最後一條。要疊加多段用 `add:=1`。
- 樣式只能「線條」或「線段」：**線條**會把終點沿斜率延伸到最新 bar（射線效果，只延終點不延起點）；**線段**只畫指定範圍。
- 起點永遠在 X 較小側（`PlotLine(1,5,5,10,10)` == `PlotLine(1,10,10,5,5)`）；起訖 X 相同 = 水平線（換日只需呼叫一次即可畫滿整日）；清線用 `PlotLine(1,0,0,0,0)`。
- 底層仍是 Plot：序列編號須為 constant 不可動態；每個 X 只有一個 Y（**無法畫垂直線**）。

### PlotK（自訂 K 棒）

`PlotK(序列編號, Open, High, Low, Close [, "名稱"])`。某次洗價未執行到 PlotK 則該 bar 不畫，要清除用 `NoPlot`。**2025-06-30 起柱體與外框顏色可在繪圖設定分別修改**（舊版「不支援指定顏色」已過時）。取代主圖 K 棒的技巧：把主圖 K 線改「連線圖」且連線色設同背景，再把 PlotK 指標設主圖疊圖。可畫 Heikin-Ashi，或把他檔 OHLC（GetSymbolField）丟進 PlotK 做跨商品比較（各除同一起始 bar 價以對齊起點）。

### PlotFill（區塊填色）

`PlotFill(序號, vFrom, vTo [, "名稱"])` 把**相鄰兩次呼叫**的上緣連上緣、下緣連下緣圍成區塊：**第一次執行只畫垂直線不填，第二次起才填**。

- 🚨 兩次呼叫的 `vFrom`/`vTo` 順序相反（一次 100,200 一次 200,100）→ 填色**交叉成蝴蝶結**而非矩形。兩次呼叫間插 `NoPlot(序號)` 可切斷該段填色。
- PlotFill 在**查價視窗不顯示數值**，只有填色 — 需要邊緣數值要另外用 Plot 畫。

### 進出場訊號座標軸（免乘係數的官方做法）

繪圖設定選「**點**」樣式後，座標軸多出「置頂/置底/最高/最低」四選項，免去舊做法（把訊號值乘某數字才能定位）。置頂/置底 = 主圖最上/最下；最高/最低 = 該 K 棒上/下方。搭配：進場 `plot1(low*0.98, "entry")`、出場 `plot2(high*1.02, "exit")`，樣式改「點」挑 ▲▼。
