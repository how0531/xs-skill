...
## 13. 自動交易與回測的官方眉角（XQ lesson 教學站彙整）

> 錯誤代碼另見 `references/error-codes.md`（自動交易 1000–1501）。

### 委託／部位機制

- **`Position` = 策略目標部位（＝送出的委託單），`Filled` = 帳號實際成交部位。** 進出場只判斷 `Filled`：委託未成交而條件又成立時，會在「同一張委託書上改價」（可刻意用來每次追最新價：`if Filled=0 and 進場 then SetPosition(1)`）。只判斷 `Position`：買單未成交而出場條件成立會直接刪單，條件頻繁觸發會反覆「下單→刪單」，官方不建議。要委託價穩定就進出場都判 `Position=Filled`；要每次追價就只用 `Filled`。
- **刪未成交委託用 `CancelAllOrders()` / `CancelAllOrders(label:="標記")`，不要用 `SetPosition(Filled)`**：後者若刪單當下剛好部分成交，`Filled` 已變動，`SetPosition` 會反手送單把部位拉回目標，造成非預期成交。`CancelAllOrders` 只刪未成交、保留已成交。**重點語意：送出後不會立即改 `Position`，且刪單確認前任何 `SetPosition` 都不立即執行 — XS 會「記住最後一次」`SetPosition` 值，待刪單完成依最新 `Filled` 再執行（不是丟棄）。** 判斷刪單完成用旗標 `State=1` 搭配 `Position=Filled`。
- **收盤後系統自動把 `Position` 調成 `Filled`**（委託僅當日有效，確保隔日開盤兩者一致；整日盤期貨在夜盤結束時也做此調整）。**IOC** 未成交會被交易所取消，系統收到通知後自動把 `Position` 改回原值（否則永不同步）。**目前不支援 FOK 委託條件。** 寫跨日波段/IOC 策略別自行硬設 `Position`。

### 暖機區間必跳過下單

- 自動交易啟動時的「**資料讀取區間**」（讀 N 根歷史 K 棒暖機）內任何交易指令都不執行、`Position` 恆為 0、執行紀錄會出現錯誤提示。用 `if GetInfo("TradeMode")=0 then return;` 偵測並跳過下單。
- 若啟用「策略部位計算功能」，會進入「**策略部位計算區間**」，此區間交易指令以歷史資料**模擬撮合**（`Position`/`Filled` 會動但不送真實委託），用來讓波段策略判斷當下是否已在場內。

### 回測撮合規格（解釋「成交價/量與直覺不符」）

- **成交判斷落在「下一根（或下一筆）K 棒」**：買進市價→成交在開盤價；限價 `P>=開盤價`→成交在開盤價；`P>=最低價`→成交在委託價 `P`；`P<最低價`→不成交等下一根。賣出對稱（看最高價）。單根成交量上限 = `Min(委託量, 該 K 棒成交量)`，未足量遞延下一根。
- 🚨 **「模擬逐筆洗價」把 K 棒拆成 O/H/L/C 四筆 tick，每筆量僅 1/4**，若在 H（最高）那筆送市價買單，實際會成交在之後的最低價 L → 回測過度樂觀。修正：自動交易回測勾「**觸發即判斷成交**」（觸發當下那筆 tick 價立即成交），選股/雷達可選成交「當期收盤價」。**寫回測前務必確認此選項。**
- **總報酬率算法依產品而異，不可混用**：選股/雷達回測不含資金，用「**時間加權報酬率**」（每日再平衡）；自動交易回測有資金，用「**最大投入報酬率**」= 淨利 / 最大投入金額。獲利因子 = 毛利/ABS(毛損)×SIGN(淨利)：只賺不賠回 `+0`、只賠不賺回 `-0`、無交易回 `n/a`，一般 >1.5 才堪用。「交易分析」頁顯示的進出場價是回測頻率原始點位，**非計算報酬所用的還原價**，拿它自行驗算會對不上。

### 實單細節

- 連續月份商品（`FITX*1.TF`）實際以最近月份合約送委託；指定月份商品到期時系統自動停止該商品執行。自動交易回測有「**每日部位歸零**」選項，當沖勾了免自己寫收盤平倉。
