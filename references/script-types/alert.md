...
> ✅ 已查證：`BidAskFlag`=內外盤、`BidPrice`=買入價、`AskPrice`=賣出價、`SeqNo`=當日序號（官方搜尋 API ename 對應，中英文皆可用）。`TickGroup` 也確認存在（官方教材「台股逐筆撮合的連續成交Tick序列」），標示逐筆撮合下每筆成交的位置，5 種值：
> - `-1` 非逐筆撮合（開盤第一筆、收盤那筆）
> - `0` 逐筆撮合但只撮出單筆
> - `1` 連續成交序列的第一筆、`2` 中間任一筆、`3` 最後一筆
>
> 用途：一筆大額委託吃掉多檔委賣會產生「連續成交序列」（多筆不同價的成交同屬一筆委託），靠 `TickGroup` 把整串找出來加總，才是這筆的真實力道。`ReadTicks` 會自動把連續成交序列合併成一列（省去手動用 TickGroup 拼），故有 ReadTicks 時通常不需自己讀 TickGroup。
...
## 策略雷達的官方眉角（XQ lesson 教學站彙整）

> 錯誤代碼另見 `references/error-codes.md`（雷達即時 5023–5032、雷達回測 4–34）。

### 觸發頻率由面板「觸發設定」決定，不是腳本

同一支 `ret=1` 腳本，實際通知頻率看雷達面板「觸發設定」四模式：

- **連續觸發**：每次成交/洗價符合都通知
- **K 棒內單次觸發**：同一根 K 棒只通知第一次，換 K 棒可再通知
- **啟動後只觸發一次**：整個執行期間只通知第一次
- **單次洗價模式**：只把「最後一根 K 棒」符合的商品列出即停止，執行邏輯類似選股中心 — 可拿逐筆洗價的警示腳本當「即時掃描一次」用

同一段腳本配不同觸發設定，結果數量差很多。

### 跨商品前提條件的兩種做法（計算跟誰的行情走不同）

- **基準商品參照**（雷達設定畫面右方按鈕）＝等於同時跑另一支獨立雷達，只有基準商品先符合、原雷達觸發才發出。不用改腳本，計算跟著**基準商品**行情更新。
- **腳本內** `GetSymbolField("TSE.TW", "收盤價")`（TSE.TW=加權指數）取值、末參數指定頻率做跨頻率 — 計算跟著**觸發商品（當前個股）**行情更新，較精確但要改腳本。
- 兩商品報價更新時間差，即使邏輯相同結果仍可能略不同。

### 自動洗價的送單與重洗機制

- 自動洗價可在盤前/無成交時運算，但**「送單」只在券商收單時段執行**，超出則紀錄顯示「非收單時段」且交易指令不執行。
- **換 Bar 後第一次自動洗價會重洗已結束的 K 棒，該次不觸發警示或交易。** 自動洗價不產生新 K 棒；商品持續無成交時運算用的仍是最後一次成交的 K 棒。間隔可設 1~60 秒。
- 期貨盤前 2 分鐘不能刪改單、證券盤前不能下市價單；雷達/自動交易的期貨一律送「範圍市價單」。

### 回測解讀

- 🚨 **回測「交易方向」預設**：雷達沒設方向時系統預設「**作空**」（非直覺的作多）。回測起訖日預設近 3 個月，重啟後回復預設、自訂區間不保留 — 回測前務必明設方向與區間。
- **整體統計用時間加權報酬率（TWR）**，項目名後標「**(單筆)**」者才是單一交易逐筆進出的報酬 — 兩套數字別混。
- 「模擬逐筆洗價」：日頻回測用「當天 1 分鐘 K 棒」洗價、週/月頻用「日資料」洗價；不勾則只用每根 K 棒最後的開高低收量。進/出場價只有「下期開盤價」或「當期收盤價」兩選項。
- 6.20 起回測「出場方式」可指定「**出場腳本**」（用另一支警示腳本做客製化出場），四種出場至少勾一種。回測「證券單邊交易費用」要換算：例手續費 3 折 + 交易稅 0.3%，單邊 ≈ 0.19% = `((0.1425%×0.3×2)+0.3%)/2`。

### 跨「分鐘＋日＋週」三重濾網要拆腳本串接

單一雷達腳本**無法**在一支內做「日線+週線+盤中分鐘」多頻率濾網（選股只支援日線以上、不含分鐘/Tick）。標準做法：把日線以上濾網寫成**選股腳本**設每日自動執行，結果餵給一支**盤中洗價的策略雷達**當第三重（Triple Screen）。

### 看盤常用但欄位表未列的即時報價欄位

`q_CashDirect`（佔大盤成交比重%）、`q_BoughtLotsAtOpen`（開盤委買張數）、`q_BoughtAverageAtOpen`（開盤委買均張）、`q_BidAskDiff`（委買賣差）、`q_UpSecurities`（上漲家數）、`q_UpLimitSecs`（漲停家數）、`q_OutSize`/`q_InSize`（外/內盤比）。可做盤中強勢股掃描。
