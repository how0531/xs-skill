...
> 來源：https://xshelp.xq.com.tw/XSHelp/lists?a=FIELDFUNC（官方 XSHelp，自動爬取）

## CheckField（判斷欄位數值是否合法）

**語法**：判斷欄位資料是否存在，回傳 True / False。
CheckField("外盤量", "D");
...
**語法**：判斷指定商品的欄位資料是否存在，回傳 True / False。
...
**語法**：讀取系統內欄位的財務季別。傳回數值格式。
欄位的財務季別 = GetfieldFiscalQ("欄位名稱")
欄位的財務季別 = GetfieldFiscalQ("欄位名稱", "頻率")
傳入欄位名稱與指定的欄位頻率。
...
**語法**：讀取系統內欄位的財務年度。傳回數值格式。
欄位的財務年度 = GetfieldFiscalY("欄位名稱")
欄位的財務年度 = GetfieldFiscalY("欄位名稱", "頻率")
傳入欄位名稱與指定的欄位頻率。
...
## GetSymbolFieldTime（取出欄位最新一期的日期）

**語法**：讀取系統內的欄位的資料時間
欄位資料日期 = GetSymbolFieldTime("ID", "欄位名稱")
欄位資料日期 = GetSymbolFieldTime("ID", "欄位名稱","頻率")
傳入欄位名稱，跟指定的欄位頻率。如果不傳頻率的話，則使用目前執行頻率。回傳欄位的資料時間，格式為HHmmss 8碼數字。
...
- 合約乘數：回傳期貨/選擇權的契約價值。格式是數值。支援台灣期貨和台灣選擇權。

## IsSupportField（判斷欄位是否支援）

**語法**：判斷傳入的欄位和頻率是否為可支援的欄位。
...
## IsSupportSymbolField（判斷欄位是否支援）

**語法**：判斷傳入的商品代碼、欄位和頻率是否為可支援的欄位。
