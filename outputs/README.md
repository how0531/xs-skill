...
**2. 360 分鐘頻率不支援，且頻率不能做成參數。** XS 官方 `GetField` 頻率代碼為 1／2／3／5／10／15／20／30／45／60／90／120／135／180／240 分鐘，沒有 360，故取最接近的 **240**。

而且頻率**必須寫成字串常數**：`GetField` 與 `SetBarBack` 的頻率參數決定初始化階段要載入哪一個資料序列，傳入 `input` 變數會編譯失敗（實測錯誤訊息：「函數 GetField：第 2 個參數應該是 String」、「函數 tBackBar：第 2 個參數應該是 String」，並連帶讓外層 `XAverage` 報「需要輸入 2 個參數」）。腳本內三處以 `// 【通道頻率】` 標記，要換頻率請同步修改。

若真的需要在 UI 上切換頻率，只能用 `if _Mode = 1 then begin … "240" … end else begin … "120" … end;` 逐分支寫死字面值。代價是 `GetField` 的預讀取機制不受 `if` 控制，所有分支的資料都會被載入。
...
5. **通道頻率已寫死為 240**：不是設計選擇而是平台限制，見上方第 2 點。
